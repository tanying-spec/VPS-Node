...
> `0.2.0-dev.87` 加固后台监测安装与卸载：安装先展示调度方式、频率和目标，要求输入 `ENABLE`；运行器及 systemd/OpenRC 定义带所有权标记，发现同名外部文件立即拒绝覆盖。所有文件同目录暂存并在确认后绑定目标状态，systemd 启用失败会恢复安装前文件与定时器状态；卸载只停止和删除能证明属于 VPS-Node 的监测定义，外部同名任务原样保留。

> `0.2.0-dev.86` 将网络一致性接入低开销后台自愈：定时检查会记录运行时漂移、持久化损坏、孤立回滚记录和缺少回滚点的配置，并在恢复一致后记录恢复事件；后台始终拒绝写 sysctl、覆盖或删除网络文件。相同网络状态连续检查只记录一次，避免每 5/15 分钟刷屏。

> `0.2.0-dev.85` 补全首页“导出脱敏诊断”的证据链：报告现在记录网络优化六态、实时拥塞控制/qdisc、仅在严格有效时出现的持久化目标，以及机器可读的建议动作。报告不会写入 sysctl 文件路径、回滚前参数、IP、域名、Token、UUID 或完整节点链接；每种网络状态均通过独立报告与 SHA-256 校验测试。

> `0.2.0-dev.84` 让首页总体结论纳入网络优化一致性：在节点服务可用后，持久化损坏、配置缺少回滚点、孤立回滚记录和实时参数漂移不再被“主线路/主备线路已就绪”掩盖。首页分别给出准确状态，并映射到 `4 → 2` 查看、`4 → 5` 修复、`4 → 6` 回滚或 `5 → 3` 导出诊断的完整编号路径；正常一致或未应用时不会误报警。

> `0.2.0-dev.83` 为主机网络配置增加严格所有权边界：只有包含唯一 VPS-Node 标记、两项受管参数各出现一次且无额外内容的普通文件才会被识别。网络测速应用与回滚在操作前拒绝外部、损坏或孤立配置；测速期间持续绑定配置文件和实时 sysctl 状态，其他任务发生修改时零覆盖中止。系统已经自行启用 BBR/fq 时只说明“不由 VPS-Node 管理”，不会伪造回滚点。
...
> `0.2.0-dev.79` 为手工网络回滚增加全局影响预览和 `ROLLBACK` 确认：展示拥塞控制及队列规则的当前值与恢复值，并说明会删除 VPS-Node 持久化网络配置。确认绑定回滚记录、配置文件和实时 sysctl 状态；期间任一项变化都会中止，卸载内部的 `--quiet` 恢复仍保持非交互。

> `0.2.0-dev.78` 完善节点删除预览：`vp delete` 会在确认前显示名称、协议、监听端口、将同步删除的凭据轮换记录数，以及永久移除新旧凭据并重启 Mihomo 的影响。取消统一返回未执行状态，节点、轮换记录和生成配置保持不变。

> `0.2.0-dev.77` 为 Reality/Argo 节点创建增加统一预览和 `CREATE` 确认：先展示名称、监听端口、SNI/地址族或 Tunnel 主机名/路径及服务影响，再生成 UUID、Reality 密钥、Short ID 或随机 WebSocket 路径。取消时不会生成任何新凭据，也不会创建配置事务。

> `0.2.0-dev.76` 为节点编辑增加脱敏变更预览：`vp edit` 会显示协议、名称、端口、SNI/地址族或 Tunnel 主机名/路径的旧值与新值，并明确 UUID 和 Reality 密钥不变；只有输入 `EDIT` 才重新生成配置和重启 Mihomo，预览不会输出任何凭据或私钥。

> `0.2.0-dev.75` 为凭据轮换开始阶段增加统一确认：`vp rotate` 会展示节点、协议、宽限小时数、新旧凭据并存影响及准确后续命令，只有输入 `ROTATE` 才生成新 UUID 和重启 Mihomo。取消发生在随机凭据生成和事务创建之前，保证节点、配置与轮换数据库零写入。

> `0.2.0-dev.74` 将异常中断纳入内核安装事务：Mihomo/cloudflared 在二进制提交后会注册专用 `EXIT/HUP/INT/TERM` 清理器。脚本被终止或最终状态提交失败时，会立即恢复旧二进制、原回滚点、运行参数、Token、服务运行器和配置事务，并清除敏感临时文件。

> `0.2.0-dev.73` 完整保留二进制灾难恢复链：Mihomo/cloudflared 更新前会暂存安装前已有的回滚点；若运行参数、状态事务、配置验证、服务安装或重启失败，会同时恢复旧当前版本和原回滚版本。只有全部提交成功后，原当前版本才正式成为新的回滚点。

> `0.2.0-dev.72` 将安装确认绑定到当前与回滚二进制的精确状态：Mihomo/cloudflared 候选完成验证后会重新复核，期间出现的并发修改会保留并使安装失败。Tunnel 在候选尚未提交时失败不再错误调用旧备份恢复，避免用更早版本覆盖外部新文件或 Token。

> `0.2.0-dev.71` 补齐二进制回滚安全边界：`vp core-rollback` 与 `vp tunnel-rollback` 在验证备份后展示当前/目标版本及服务影响，并要求输入 `ROLLBACK`。确认绑定当前与备份二进制的精确文件状态；验证后被其他任务替换的备份会保留并被拒绝，不会回滚到过期对象。

> `0.2.0-dev.70` 统一内核安装的危险操作入口：`vp core-install` 与 `vp tunnel-install` 会先展示当前版本、可信来源、校验方式和服务影响，并要求输入 `INSTALL`。取消发生在依赖安装、目录初始化、下载或 Token 读取之前，保证项目状态零写入。

> `0.2.0-dev.69` 为 CLI 回滚增加统一的最终确认：`vp rollback` 在完整性校验后展示当前版本、目标版本和校验结论，并要求输入 `ROLLBACK` 才会交换脚本。取消发生在创建目录或暂存文件之前，保证当前 CLI、回滚脚本和校验文件零写入。
