...
所有需要真实 Mihomo、OpenRC、网络出口或资源压力的实机测试只能在 `134.209.180.134` 运行。`dev.96` 已完成完整 smoke、安装测试、NAT/CDN API 隔离测试、整体卸载远端对象恢复测试、真实 CDN 验收器默认路径和实机隔离验收；真实 Cloudflare CDN 边缘仍需独立测试材料。原始脱敏证据摘要见 `REAL_HOST_EVIDENCE.md`。
