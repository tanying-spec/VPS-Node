...
| 数据库约束、配置漂移、自愈互斥 | 节点删除确认状态绑定、并发节点保留、`tests/smoke.sh` | 已通过 |
...
早期开发阶段曾记录 Reality/VLESS-WS、Cloudflare Tunnel、4×10 MiB 压力和约 16 MiB/s Tunnel 吞吐结果。这些结果可以作为设计参考，但不是 `0.2.0-dev.64` 当前代码的验收证据，因此不用于宣称当前版本已通过。
