...
所有需要真实 Mihomo、OpenRC、网络出口或资源压力的实机测试只能在 `134.209.180.134` 运行。`dev.92` 已完成完整 smoke、安装测试、CLI 更新/回滚、隔离监控定义、真实 DNS 和实机隔离验收；`dev.91` 已完成一键维护验收；`dev.90` 已完成统一只读预检。原始脱敏证据摘要见 `REAL_HOST_EVIDENCE.md`。
