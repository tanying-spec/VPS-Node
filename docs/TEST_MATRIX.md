...
| 安装、更新、回滚、来源与降级保护 | 本地候选源、顶层 SHA 样本、同版本异内容/隐式降级拒绝、显式降级、初始化失败恢复及 SHA-256 篡改故障注入 | 已通过 |
...
早期开发阶段曾记录 Reality/VLESS-WS、Cloudflare Tunnel、4×10 MiB 压力和约 16 MiB/s Tunnel 吞吐结果。这些结果可以作为设计参考，但不是 `0.2.0-dev.33` 当前代码的验收证据，因此不用于宣称当前版本已通过。
