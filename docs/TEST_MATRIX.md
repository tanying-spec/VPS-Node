...
| 后台自愈锁与网络监测 | 活跃锁跳过、死亡 PID 回收、PID 相同但启动时间不符时状态识别与安全回收；网络漂移、损坏、两类孤立状态及恢复事件；重复执行单事件、sysctl/配置/快照哈希不变、健康日志去重 | 已通过 |
...
早期开发阶段曾记录 Reality/VLESS-WS、Cloudflare Tunnel、4×10 MiB 压力和约 16 MiB/s Tunnel 吞吐结果。这些结果可以作为设计参考，但不是 `0.2.0-dev.86` 当前代码的验收证据，因此不用于宣称当前版本已通过。
