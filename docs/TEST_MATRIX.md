...
| 可恢复卸载及文件残留 | 外部恢复包与 SHA-256、服务停止失败零删除/零网络回滚、Mihomo/cloudflared 进程级退出门槛、孤立 Tunnel 拒绝、OpenRC watchdog 暂存恢复、隔离目录完整卸载 | 已通过自动验证 |
...
早期开发阶段曾记录 Reality/VLESS-WS、Cloudflare Tunnel、4×10 MiB 压力和约 16 MiB/s Tunnel 吞吐结果。这些结果可以作为设计参考，但不是 `0.2.0-dev.57` 当前代码的验收证据，因此不用于宣称当前版本已通过。
