...
所有需要真实 Mihomo、OpenRC、网络出口或资源压力的实机测试只能在 `134.209.180.134` 运行。`dev.91` 已完成隔离功能与一键维护验收；`dev.90` 已完成统一只读预检；`dev.89` 已完成真实 DNS 验收。原始脱敏证据摘要见 `REAL_HOST_EVIDENCE.md`。
...
| 一键安全维护 | dry-run/取消零写入、确认前恢复点、日志和过期临时项清理、损坏数据库拒绝且保留恢复点 | 自动测试与 `dev.91` 唯一测试机隔离验收通过 |
...
| 后台自愈锁、网络监测与安装 | 活跃锁跳过、死亡 PID/PID 复用回收；网络异常及恢复、重复执行单事件、sysctl/配置/快照哈希不变；监测安装取消零写入、外部 runner/timer 拒绝、所有权标记与权限、systemd enable 失败恢复三文件哈希 | 已通过 |
| 首页结论与入口 | 未安装、数据异常、轮换中/到期、残留 Token 无 Argo 节点；网络持久化损坏、孤立配置、孤立回滚记录、运行时漂移及正常无误报；下一步完整映射到主菜单与子菜单编号 | 已通过 |
| 旧项目迁移输入一致性 | 源符号链接拒绝、确认前后 SHA-256 不变、确认后端口重检、源变化时零节点/零备份副作用 | 已通过 |
| 数据库约束、配置漂移、自愈互斥 | 节点编辑/删除状态绑定、并发节点保留、零事务残留、`tests/smoke.sh` | 已通过 |
| DNS、内存/CPU 档位、端口冲突 | 模拟 cgroup、DNS 和监听端口 | 已通过模拟验证 |
| 网络优化状态、所有权与漂移修复 | 无文件、实时一致、运行时漂移、持久化缺项/重复项/无所有权标记、孤立回滚记录、孤立配置；外部文件零覆盖、测速中并发变化零写入、外部 BBR/fq 不伪造回滚点；漂移修复取消零修改、双参数部分失败补偿、成功后文件哈希不变 | 已通过自动验证 |
| 备份、恢复、恶意归档拒绝 | 真实 tar 往返与攻击样本 | 已通过 |
| 恢复校验默认策略 | 缺少 SHA-256 默认拒绝、显式旧备份兼容、归档/sidecar 符号链接拒绝、解压后/确认后归档与 sidecar 状态复核，兼容模式仍执行归档与数据库约束 | 已通过 |
...
| Reality 真实认证与并发 | `tests/isolated_acceptance.sh` | `dev.91` IPv4 2/2 路通过 |
| Alpine/OpenRC 服务生命周期 | `tests/isolated_acceptance.sh` | `dev.91` 创建、运行、维护、卸载闭环通过 |
| 正式 Mihomo/cloudflared 前后不变 | 状态及配置/init 哈希对比 | `dev.91` PID、镜像、命令行及全部受检摘要不变 |
...
早期开发阶段曾记录 Reality/VLESS-WS、Cloudflare Tunnel、4×10 MiB 压力和约 16 MiB/s Tunnel 吞吐结果。这些结果可以作为设计参考，但不是 `0.2.0-dev.87` 当前代码的验收证据，因此不用于宣称当前版本已通过。
