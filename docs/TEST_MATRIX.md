...
| 网络优化状态真实性 | 无文件、实时一致、运行时漂移、持久化缺项、孤立回滚记录、孤立配置；首页与网络页复用同一检查结果 | 已通过自动验证 |
...
早期开发阶段曾记录 Reality/VLESS-WS、Cloudflare Tunnel、4×10 MiB 压力和约 16 MiB/s Tunnel 吞吐结果。这些结果可以作为设计参考，但不是 `0.2.0-dev.81` 当前代码的验收证据，因此不用于宣称当前版本已通过。
