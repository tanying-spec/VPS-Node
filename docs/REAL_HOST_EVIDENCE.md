...
- 2026-07-25 07:15 UTC 在唯一授权主机完成完整 smoke、安装器和隔离真实 Mihomo 验收。
- Cloudflare HTTP 200/`success:false` 语义失败测试通过：DNS/Ruleset 仍存在时删除被拒绝，本地节点与恢复记录保持不变。
- 正式 Mihomo 与 Cloudflare Tunnel 保持 `active`，受检 PID、进程镜像、命令行、配置/init 和敏感状态摘要前后不变。
- 测试脚本 SHA-256：`2f8d1f70637b602e3d94ba5fbdfae3388194733eb4353129aeabeb9a27488262`。
- 原始脱敏证据 SHA-256：`e6dacec286807af3b676e99a82a881b017402565fffa6a643407220e000dfb0d`。
- 真实 CDN 公网边缘仍需独立测试材料。

## 0.2.0-dev.97

- 2026-07-25 07:06 UTC 在唯一授权主机完成完整 smoke、安装器和隔离真实 Mihomo 验收。
- 直连 CDN 创建、VLESS-WS 链接生成和错误的独立公网端口更新回滚测试通过；NAT CDN 端口事务保持通过。
- 正式 Mihomo 与 Cloudflare Tunnel 保持 `active`，受检 PID、进程镜像、命令行、配置/init 和敏感状态摘要前后不变。
- 测试脚本 SHA-256：`ebd8c437cb9bf39b3c61ecd51eecc8cb187e664fb9596a42cb41422904131c86`。
- 原始脱敏证据 SHA-256：`9d2ef2e6fd11f170c303eaffa80a51dcba767d4a3279b02263714a697fde029c`。
- 真实 CDN 公网闭环仍需独立测试材料，不能用本次本地/伪 API 证据替代。
